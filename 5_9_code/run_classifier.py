...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    """
    filter(function,iterable):用于过滤序列，过滤掉不符合条件的元素，返回由符合条件的元素组成的新列表
    iterable中的每个元素作为参数传递给函数进行判断，返回true/false，最后将返回true的元素放到新列表中输出
...
def main():
    ...
    ## Required parameters
    """data_dir:存储数据集的基本地址（不包括数据集名）"""
    parser.add_argument("--data_dir", default=None, type=str, required=True,
                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")
    """model_type:模型的类型-albert"""
    parser.add_argument("--model_type", default=None, type=str, required=True,
                        help="Model type selected in the list: ")
    """模型的名称或路径:预训练模型的路径:'prev_trained_model/albert_small_zh'"""
    ...
    """vocab文件:'prev_trained_model/albert_small_zh/vocab.txt'"""
