...
class AlbertEmbeddings(nn.Module):
    ...
    """
    embedding层
    embedding=input_id_embedding(给定)+token_type_embedding(给定)+position_embedding
...
class AlbertAttention(nn.Module):
    """
    AlbertAttention的输出有两个：
    1.第一个就是attention应该得到的输出，即多头汇聚后经过权重层的输出
    2.第二个是一个tuple类型，tuple中的第一个是多头汇聚后未经权重层的输出（self-attention的输出），
                                  第二个是self-attention的attention-probs输出（如果output_attention=true的话）
    """
    def __init__(self, config):
...
class AlbertLayer(nn.Module):
    """
    AlbertLayer有两个输出：
    1.第一个是ffn的输出，也就是input经过该层layer后的最终输出
    2.第二个是tuple，tuple中的第一个是未经权重层的self-attention的多头汇聚，第二个是attention_probs
    """
    def __init__(self, config):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        """
        outputs[0]:ffn层的输出，也就是input经过该layer后的最终输出：batch_size,sequence_length,hidden_size
        output[1]:(tuple):(self_attention未经Linear projectiob的context,attention_probs)
        """
        return outputs

class AlbertGroup(nn.Module):
    """
    tuple:tupe中有两个元素：1.每个layermodule的ffn的输出构成的tuple 2.每个layermodule中self-attention的context和attention_probs构成的输出
    outputs[0]:inner_group_num个ffn的输出构成的tuple
    output[1]:inner_group_num个self-attention的context和attention_probs构成的tuple
    outputs[1][0]:第一个layermodule中的context和attention_probs构成的tuple
    """
    def __init__(self, config):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        for inner_group_idx in range(self.inner_group_num):
            ...
            """hidden_states:是layer中前一个module的ffn的输出"""
            hidden_states = layer_outputs[0]
            """layer_attentions:tuple(context,attention_probs)"""
...
class AlbertTransformer(nn.Module):
    """
    输出有三个：
    1.outputs[0]:第一个是最后一个ffn的输出
    2.oupputs[1]第二个是emb_hidden_state+num_hidden_groups*num_inner_group个hidden_states
        outputs[1][0]:embedding_hidden_state
        outputs[1][1]:第一个group的第一个module的hidden_state
        outputs[1][2]:第一个group的第二个module或者第二个group的第一个module的hidden_state
    3.outputs[2]:num_hidden_groups*num_inner_group个(context,attention_probs)构成的tuple
    """
    def __init__(self, config):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        for layer_idx in range(self.num_hidden_layers):
            ...
            #??????head_mask[layer_idx],要给head_mask???head_mask=None时，是不是应该删掉
...
class AlbertMyClassifier(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.inner_group_num==1:
            ...
            raise ValueError("inner_group_num!=1,拼接的不是所有group最后一个ffn的输出")

        self.classifier = nn.Sequential(nn.Linear(num_inputs, config.hidden_size),
                                        nn.ReLU(),
                                        nn.Linear(config.hidden_size,config.num_labels)
                                        )
        # self.classifier=nn.Sequential(nn.Linear(num_inputs,config.num_labels))
