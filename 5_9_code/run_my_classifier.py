...
from model.modeling_albert import AlbertMyClassifier,AlbertForSequenceClassification
...
def train(train_dataset, model, tokenizer,per_gpu_train_batch_size,local_rank,device,n_gpu,max_steps,gradient_accumulation_steps,
    ...
    optimizer = AdamW(params=optimizer_grouped_parameters,eps=adam_epsilon)
...
# model_name_or_path='outputs/tnews_output_onelayer/checkpoint-5000/checkpoint-10000/checkpoint-15000'
# model =AlbertMyClassifier.from_pretrained(model_name_or_path,
#                                                        from_tf=bool('.ckpt' in model_name_or_path),
#                                                         config=config)
# output_dir='outputs/tnews_output'
...
output_dir='outputs/tnews_output_two_layer'
num_train_epochs=30
...
#         model =AlbertMyClassifier.from_pretrained(checkpoint)
#         model.to(device)
...
if is_evaluate:
    ...
    for root,dir,files in os.walk(output_dir):
        if files:
            ...
            model=AlbertMyClassifier.from_pretrained(root)
            model.to(device)
            result =evaluate(model,task_name,output_dir,data_dir,model_name_or_path,max_sequence_length,local_rank,tokenizer,
                             per_gpu_train_batch_size,n_gpu,device,output_mode)
